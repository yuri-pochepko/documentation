...
1. Since we modified `composer.json` we will need to update Composer. This will also download the defined dependencies:
    ...
    ![image of terminal running a composer install](/source/images/guides/drupal-8-commerce-no-ci/drops-8-composer-update.png)
...
5. Now let’s run `git status` again to make sure everything is included:
   ...
   ![Image of git status showing the changed files in red](/source/images/guides/drupal-8-commerce-no-ci/drops-8-composer-git-status-after-installing-d8.png)
...
5. Running `git status` should show new files `commerce_base` and its `dependencies`:
   ...
   ![Git Status showing updated Composer files](/source/images/guides/drupal-8-commerce-no-ci/git-status.png)
...
2. Log in to your Drupal site in the Dev environment. The presence of the **Commerce** button on the toolbar indicates a succefull install:
    ...
    ![Drupal Commerce in the Toolbar](/source/images/guides/drupal-8-commerce-no-ci/commerce-button.png)
